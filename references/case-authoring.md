...
**实操判别**：从 skill 的**私有资产**出题——它独有的框架名/标签体系/特定流程/专有数据结构（如某分析 skill 的 22 维标签、异常信号卡），不从"通用能力"出题（结构化分析/分点总结/仔细认真——这些裸模型原生就会）。典型反例：考"会不会结构化分析"的 case，裸模型满分、Δ=0，好 skill 险被误判——这个坑极易踩：考题角度稍有偏差，测到的就是通用能力而不是 skill 的独有价值。
...
- **judge 误判防范**（官方文档警告）：`tool_used: Skill` 过了但 Δ 为负 → 先怀疑 judge 再怀疑 skill。重要结论需确认时，直接给本脚本显式传 `--judge-model <judge-model>`；它会与 execution model 分开透传，绝不会把 `--model` 自动复制给 judge。

## 判定与证据链

| 信号 | 判定 |
|---|---|
| with ≈ without（双高分）且 case 只考了通用能力 | **inconclusive**——case 区分度不足，重新出题（考私有资产）；禁止在此 case 上下"原生覆盖"结论 |
...
| 两份答卷都低分 | **inconclusive**——case 或 grader 出问题，重新出题 |
...
- **本地存储**：`<out-dir>/cases/<skill-instance-id>/`（prompt.md + graders/，即官方 eval 目录格式，可复跑；旧 logical id 路径仍可被唯一解析时兼容读取，但建议迁移）
- **新模型发布重扫**：评测缓存按 `skill 内容 + case/graders + runs + execution model + judge model + ablation + Claude 版本 + eval schema` 的完整 fingerprint 生效——换模型、改 case、改 runs、换 judge、换 Claude Code 版本都会自动失效重考；`model=unknown/mixed` 的结果永不缓存。raw 证据按 `eval-raw/<instance>/<model>/<fingerprint>/attempt-N.json` 隔离，绝不复用旧 raw。
