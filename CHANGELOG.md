...
- 报告骨架面向普通使用者重写表头与口径声明：全部表格列头、证据等级与扫描状态枚举渲染成中文，报告顶部内置「名词速查」（常驻/触发/参考文件/token/证据等级）
- 报告判读规范新增「读者预设与说话方式」硬约束：读者是装了一堆 skill 的普通使用者；注入层术语必须翻译、数字必须跟后果、建议必须写给非工程人员可执行
- README 案例展示按非技术读者重写：先讲「名片区装不下会被默默丢掉」的背景，再讲五条核心发现；表格列头全部中文化
- 示例报告（docs/example-report.md）用新版渲染重新生成
